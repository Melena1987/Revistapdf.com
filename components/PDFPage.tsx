            ...
            try {
                ...
                if (mounted && annotations.length > 0) {
                    ...
                    if (AnnotationLayerClass) {
                         ...
                         // Select all anchors generated by PDF.js
                         const links = annotationDiv.querySelectorAll('a');
                         
                         links.forEach((link: HTMLAnchorElement) => {
                             // 1. Stop Drag/Swipe Propagation
                             // These events trigger the Flipbook page turn. We must stop them on links.
                             ['mousedown', 'touchstart', 'pointerdown'].forEach(evtType => {
                                 link.addEventListener(evtType, (e) => {
                                     e.stopPropagation();
                                     // Do NOT preventDefault here, or click might not fire on some touch devices
                                 }, { passive: false });
                             });
                             ...
                             link.addEventListener('click', (e) => {
                                 ...
                                 // Handle Internal Links (hashes)
                                 else if (href.startsWith('#')) {
                                     // Allow default behavior for internal links if managed by PDF.js or hash navigation
                                     // But usually stopping propagation is enough to let PDF.js listener run if attached to element
                                 }
                             });
